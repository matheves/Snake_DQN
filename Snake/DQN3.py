...
class DQN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.nn.functional.relu(self.bn1(self.conv1(torch.unsqueeze(x.float(), 0))))
...
class DQN_Snake:
    ...
    def epsilon_greedy_policy(self, state, epsilon=0):
        if torch.rand(1) < epsilon:
            ...
            Q_values = torch.tensor([[torch.argmax(self.model(state))]])
            return Q_values
        
    def sample_experiences(self):
        batch = random.sample(self.replay_memory, self.batch_size)
        states = torch.unsqueeze(torch.tensor(batch[0][0]), 0)
        
        actions = torch.unsqueeze(torch.tensor(batch[0][1]), 0)
        rewards = torch.unsqueeze(torch.tensor([batch[0][2]]), 0)
        next_states = torch.unsqueeze(torch.tensor(batch[0][3]), 0)
        dones = torch.unsqueeze(torch.tensor([batch[0][4]]), 0)
        
        
        for i in range(1, len(batch)):
            states = torch.cat((states, torch.unsqueeze(torch.tensor(batch[i][0]), 0)))
            actions = torch.cat((actions, torch.unsqueeze(torch.tensor(batch[i][1]), 0)))
            rewards = torch.cat((rewards, torch.unsqueeze(torch.tensor([batch[i][2]]), 0)))
            next_states = torch.cat((next_states, torch.unsqueeze(torch.tensor(batch[i][3]), 0)))
    ...
    def training_step(self):
        ...
        next_Q_values = self.model(next_states[0])
